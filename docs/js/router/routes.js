...
export function configureRoutes() {
    ...
    // Break Predict route
    router.addRoute('/break-predict', {
        title: 'Debtools - Break Predict',
        init: async () => {
            // Load the break predict HTML content
            const breakPredictContainer = document.getElementById('break-predict-tab');
            if (breakPredictContainer) {
                try {
                    // Use the new break predict component
                    const { breakPredictComponent } = await import('../features/breakPredict/breakPredict.component.js');
                    if (!breakPredictComponent.isReady()) {
                        await breakPredictComponent.init(breakPredictContainer);
                    }
                } catch (error) {
                    console.error('Error loading break predict content:', error);
                }
            }
        },
        cleanup: async () => {
            // Cleanup break predict component when leaving route
            const { breakPredictComponent } = await import('../features/breakPredict/breakPredict.component.js');
            if (breakPredictComponent.isReady()) {
                breakPredictComponent.cleanup();
            }
        }
    });
