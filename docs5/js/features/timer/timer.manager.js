...
class TimerManager {
    ...
    init() {
        if (this.isInitialized) {
            // console.log('⚠️ TimerManager ya inicializado, saltando init...');
            return;
        }
        ...
        // console.log('✅ TimerManager inicializado correctamente');
    }

    loadPersistedTimers() {
        // Si ya hay timers en memoria, solo re-renderizar sin reactivar
        if (this.timers.length > 0) {
            // console.log('⚠️ Timers ya en memoria, solo re-renderizando...');
            setTimeout(() => {
                this.timers.forEach(timer => {
                    timerView.renderTimer(timer);
                    // NO reactivar - los timers ya están corriendo en segundo plano
                });
            }, 100);
            return;
        }
        
        const savedTimers = timerState.getTimers();
        
        if (savedTimers.length > 0) {
            // console.log('📖 Cargando timers desde localStorage...');
            this.timers = savedTimers.map(saved => ({
                ...saved,
                alertsTriggered: new Set()
                // Keep original isRunning, isPaused, isFinished states
            }));
            
            // Render all saved timers with a delay to ensure DOM is ready
            setTimeout(() => {
                this.timers.forEach(timer => {
                    timerView.renderTimer(timer);
                    ...
                    if (timer.isRunning && !timer.isPaused && !timer.isFinished) {
                        console.log('🔄 Reactivando timer:', timer.name);
    ...
    saveTimersState() {
        timerState.setTimers(this.timers);
    }
    ...
    playTestBeep(audioContext) {
        try {
            ...
            // Audio test completed - no feedback needed for simple test
    ...
    fallbackAudioTest() {
        try {
            ...
            // Silent fallback - no alerts needed for simple test
            console.log('Audio test failed:', speechError);
        }
    }
    ...
    async startTimer(timerId) {
        ...
        const interval = setInterval(() => {
            ...
            // Save state every tick to maintain persistence
            this.saveTimersState();
        }, 1000);
    ...
    toggleTimerMode(timerId) {
        ...
        // Regenerate default alerts for the new mode
        timer.alerts = this.getDefaultAlerts(timer.initialMinutes, timer.initialSeconds, timer.isStopwatch);
        timer.alertsTriggered.clear();

        // Reset to appropriate starting values
        if (timer.isStopwatch) {
            // Stopwatch mode: start from 0:00
            timer.currentMinutes = 0;
            timer.currentSeconds = 0;
        } else {
            // Timer mode: start from initial time
            timer.currentMinutes = timer.initialMinutes;
            timer.currentSeconds = timer.initialSeconds;
        }
        ...
        timerView.updateTimeDisplay(timerId, timer);
        timerView.updateTimerState(timerId, timer);
        this.updateModeButtonDisplay(timerId, timer);
    ...
    updateModeButtonDisplay(timerId, timer) {
        const cardElement = document.querySelector(`[data-timer-id="${timerId}"]`);
        if (!cardElement) return;

        const modeBtn = cardElement.querySelector('.timer-btn-compact.mode-toggle');
        if (modeBtn) {
            // Force update the button text and attributes
            if (timer.isStopwatch) {
                modeBtn.textContent = 'ASC';
                modeBtn.setAttribute('data-mode', 'stopwatch');
                modeBtn.title = 'Modo Cronómetro (ASC)';
            } else {
                modeBtn.textContent = 'DSC';
                modeBtn.setAttribute('data-mode', 'timer');
                modeBtn.title = 'Modo Timer (DSC)';
            }
