...
class TimerView {
    ...
    updateModeButton(timerId, timer) {
        // This function is now handled by timerManager.updateModeButtonDisplay
        // Keeping for backward compatibility
        const timerManager = window.timerManager || (async () => {
            const { timerManager } = await import('./timer.manager.js');
            return timerManager;
        })();
        
        if (timerManager && timerManager.updateModeButtonDisplay) {
            timerManager.updateModeButtonDisplay(timerId, timer);
        }
    }
