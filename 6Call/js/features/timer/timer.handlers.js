...
class TimerHandlers {
    ...
    setupControlButtons(cardElement, timer) {
        ...
        const compactModeBtn = cardElement.querySelector('.timer-btn-compact.mode-toggle');
        const compactConfigBtn = cardElement.querySelector('.timer-btn-compact.config');
        const compactSoundBtn = cardElement.querySelector('.timer-btn-compact.sound-test');
        ...
        if (compactModeBtn) {
            compactModeBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
        ...
        if (compactSoundBtn) {
            compactSoundBtn.addEventListener('click', () => {
                timerManager.playBell(2);
            });
        }
    }
    ...
    setupRemoveButton(cardElement, timer) {
        ...
        if (removeBtnCompact) {
            removeBtnCompact.addEventListener('click', () => {
                ...
                if (timerFloating.hasFloatingWindow(timer.id)) {
                    timerFloating.closeFloatingWindow(timer.id);
                }
                timerManager.removeTimer(timer.id);
            });
        }
    ...
    setupLocalModeControls(cardElement, timer) {
        // Esta función ahora está vacía - la lógica del mode toggle se maneja en setupControlButtons
        ...
        if (floatingBtn) {
            floatingBtn.addEventListener('click', () => {
                ...
                if (timerFloating.hasFloatingWindow(timerId)) {
                    ...
                    timerFloating.createFloatingWindow(timerId, cardElement, timer);
                }
            });
        }
    }
